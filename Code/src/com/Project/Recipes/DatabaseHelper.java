...
public class DatabaseHelper {
    ...
    public static void printMenu(){
        System.out.println("Please select one of the following options:\n" +
                ...
                "U - Update ingredient stock\n" +
                "X - Exit");
    }

    /*
    Given the name of a csv file and a connection, parses file and inserts new recipes into the recipebook.
     */
    public static void importCSV(String fileName, Connection conn){
        ...
        try {
            ...
            while (fileReader.hasNextLine()) {
                ...
                if(addRecipe(data, conn)) {
                    System.out.println("Recipe added!");
                }
                else{
                    System.out.println("A recipe with that title is already in the recipe book.");
                }
            }
            fileReader.close();
    ...
    /*
    Given a line of data from a csv file, prepares for inserting into the database. This will take out the quoted data
    (in the database case, this is the list of ingredients and equipment) and store it in its own array. The method
    outputs a 2D String array.
    ...
    /*
    Asks all the questions from the user necessary to insert a new recipe.
     */
    ...
    /*
    Given a line of data, calls the proper functions to execute the SQL commands that insert a new recipe into
    the recipebook.
     */
    public static boolean addRecipe(String data, Connection conn){
        String[][] lists = getLists(data);
        String[] independentElements = lists[0];
        String[] listedElements = lists[1];
        boolean added = false;

        if(!inDatabase(conn, "RecipeInfo", "Title", independentElements[0])){
            ...
            Integer testKey = getKey(conn, "RecipeID", "RecipeInfo", "Title",
            ...
            Vector<String> columnsToFill = new Vector<String>(Arrays.asList("Title", "MainIngredient", "CuisineID", "TypeID"));
            Vector<Object> valuesForColumns = new Vector<Object>(Arrays.asList(independentElements[0], mainIngredientId, cuisineId, typeId));
            if(!independentElements[4].equals("")){
                columnsToFill.add("Servings");
                valuesForColumns.add(independentElements[4]);
            }
            if(!independentElements[5].equals("")){
                columnsToFill.add("PrepTimeMin");
                valuesForColumns.add(independentElements[5]);
            }
            if(!independentElements[6].equals("")){
            ...
            Integer recipeId = getKey(conn, "RecipeID", "RecipeInfo", "Title",
                    independentElements[0]);
            for(String ingredient : ingredients){
                addElement(conn, "Ingredients", "IngredientName", ingredient,
                        new String[]{"IngredientName, InStock"}, new Object[]{ingredient, false});
                Integer ingredientId = getKey(conn, "IngredientID",
                        "Ingredients", "IngredientName", ingredient);
                Object[] valuesToInsert = new Object[]{recipeId, ingredientId};
                addElement(conn, "RecipeIngredients",
                        new String[]{"RecipeID, IngredientID"}, valuesToInsert);
            }
            ...
            if(independentElements[8].isBlank()){ // if there is empty spot where there should be ingredients
                if(!listedElements[1].isBlank()){ // if there are entries in the ingredients part of listedElements
                    for(String equipment : listedElements[1].split(",")){
                        ...
                        Integer equipmentId = getKey(conn, "EquipmentID",
                                "Equipment", "Name", equipment);
                        Object[] valuesToInsert = new Object[]{recipeId, equipmentId};
                        addElement(conn, "RecipeEquipment",
                                new String[]{"RecipeID, EquipmentID"}, valuesToInsert);
                    }
                }
            }
            else{
                ...
                Object[] valuesToInsert = new Object[]{recipeId, equipmentId};
                addElement(conn, "RecipeEquipment", "EquipmentID", equipmentId.toString(),
                        new String[]{"RecipeID, EquipmentID"}, valuesToInsert);
            }
            added = true;
        }
    ...
    /*
    Checks to see if a value is already in a table. If it isn't, inserts the new value into the table.
...
//        System.out.println(queryString);
    ...
    tablesToJoin:
        - String - the table of the table you want to join
        - Integer - 0 if you want to do a natural join with this table; 1 if you want to do a theta join with the table
    thetaJoinColumns - this is for the JOIN ON part if we are doing a theta join; should be empty if not doing a theta
        join
    ...
    orderBy - which columns we want to order the results by
    orderBySettings - this should be ASC or DESC
    ...
    /*
    Creates the string for a join.
    tablesToJoin:
        - String - the table of the table you want to join
        - Integer - 0 if you want to do a natural join with this table; 1 if you want to do a theta join with the table
    columnsToJoin - the columns on which we will perform a theta join
     */
    ...
    // Prints the first row of a Vector<String>.
    ...
    /*
    Given an ID, prints all recipes associated with the recipe.
    ID - an ID in the database
    idType - a char indicating which kind of id was passed in (TypeID, RecipeID, etc.)>
     */
    ...
    // Prints all information for each recipes in the ArrayList.
...
//                    System.out.println("recipeInfo.length: " + recipeInfo.length);
                    ...
                    Ingredient mainIngredient = new Ingredient(mainIngredientSplit[0],
                            getBoolean(mainIngredientSplit[1].strip()));
                    ...
                    while(ingredientIt.hasNext()){
                        ...
                        ingredients[ingredientIndex] = new Ingredient(ingredientInfo[0], getBoolean(ingredientInfo[1].strip()));
                        ingredientIndex++;
                    }
                    ...
                    results = DatabaseHelper.searchJoinedTables(c,
                            ...
                            new String[]{}, new String[]{"RecipeID"}, new String[]{recipeInfoCleaned[0]}, new String[]{},
                            new String[]{});
                    Equipment[] equipmentNeeded = new Equipment[0];
                    if(results.next()){
...
//                    equipmentNeeded = new Equipment[]{new Equipment("None")};
    ...
    // Asks if the user wants to export their results to an XML. If yes, creates XML.
    ...
    // Prints facts about the recipebook.
    public static void printFacts(Connection c){
        ...
        try{
            ...
            for(int i = 0; i < queries.length; i++){
                ResultSet rs = useAggregator(c, new String[]{queries[i][0]}, queries[i][1], new String[]{},
    ...
    /*
    Executes an SQL statement that uses an aggregator.
    selections - what we want to select
    tableName - the table from which we are selecting
    columnSearch - the column(s) we are using in our search
    columnSearchValues - the values we want in the columns for our search
    orderBy - which columns we want to order the results by
    orderBySettings - this should be ASC or DESC
    groupBy - how we want to group our results
     */
    ...
    // Updates if the ingredient is in stock.
    ...
    // Given a string of "0" or "1" returns corresponding false or true
